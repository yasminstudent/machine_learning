...
// recebe o eixo (tempo ou umidade), o valor especícifco do eixo (nublado, normal) e a classe específica (sim ou não)
...
// proporção (quantas vezes a classe aparece) dividida pelo total de classes
...
function entropia(_eixo='', _valor='') {
	...
	let classesUnicas = retornaClasses(); //pega as classes/decisões
	classesUnicas.reverse();
	let multiplicacao = [];
	for(let i=0; i<classesUnicas.length; i++) { //percorre o array de classes
...
function train(config={}) {
	//verifica se os valores tem conteúdo para passar nas variáveis globais
...
function predict(entradaX=0, entradaY=0) {
	let folha = ''; //resposta/classe/decisão
...
train({
	x: ['sol', 'nublado', 'chuva', 'chuva', 'sol', 'nublado'], //tempo
	y: ['anormal', 'normal', 'anormal', 'normal', 'normal', 'anormal'], //umidade
	class: ['não', 'sim', 'não', 'não', 'sim', 'sim'] //decisão
});
