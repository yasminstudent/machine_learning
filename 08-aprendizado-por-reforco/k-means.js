/*
	O algoriítimo recebe a entrada
	retorna uma saída 
	e espera um feedback do usuário
*/
module.exports = class KMeans {
	constructor() {
		...
		//Centróides que inicialmente são aleatórios
	...
	indiceCentroide() {
		...
		let positivo = fs.readFileSync('positivo.txt', 'utf8'); //lê o arquivo positivo
		positivo = positivo.toString().trim();

		if(positivo.length > 0) { //caso tenha algum valor salvo
			const linhas = positivo.split('\n'); //transforma em array
			for(let i=0; i<linhas.length; i++) { //percorre os valores
				const indices = linhas[i].split(','); //separa o 1º valor do 2º
			...
			let negativo = fs.readFileSync('negativo.txt', 'utf8'); //lê o arquivo negativo
			negativo = negativo.toString().trim();

			if(negativo.length > 0) { //caso o arq tenha algum valor salvo
			...
			}else { //caso o arquivo não tenha nada os indices são aleatórios
				this.indice1 = Math.floor(Math.random() * this.eixoX.length);
				this.indice2 = Math.floor(Math.random() * this.eixoY.length);
			}
		}
	...
	inicializaIndices(matriz=[]) { //recebe os valores que receberam feedback negativo
		...
		for(let i=0; i<matriz.length; i++) {
			const temp = matriz[i];
			if((temp[0] == sorteio[0])&&(temp[1] == sorteio[1])) igual++; //compara se os indices são iguais
		}
		if(igual > 0) {
			if(this.chamadas < 10) {
				this.inicializaIndices(matriz); //chama a função de novo
				this.chamadas++;
			}else {
				this.chamadas = 0;
			}
		}else { //significa que foram achados valores aleatórios diferentes dos que estavam no arquivo
			this.indice1 = i1;
			this.indice2 = i2;
		}
	}
	...
	feedback() {
		...
		if(positivo.length <= 0) { //caso positivo não tenha nenhum valor registrado
			const readline = require('readline').createInterface({
				input: process.stdin,
				output: process.stdout                                                                         
			});

			//Pergunta no terminal e a resposta é guardada no feed
			readline.question('Feedback positivo ou negativo? ', (feed) => {
				let nome_arquivo = 'negativo.txt';
				if(feed.toString().toLowerCase().trim() == 'positivo') //verifica se a resposta é positiva
					nome_arquivo = 'positivo.txt';

				let antigo = fs.readFileSync(nome_arquivo, 'utf8'); //lê o arquivo antigo
				if(antigo == undefined)
					antigo = '';

				//adiciona os centroídes ao arquivo
				const novo = antigo + '\n' + this.indice1 + ',' + this.indice2;
				fs.writeFileSync(nome_arquivo, novo.toString().trim()); 

				if(feed.toString().trim().length <= 0) feed = 'negativo';

				console.log(`Seu feedback foi ${feed}.`); //caso a pessoa não digite nada
				readline.close();
			});
		}	
	}
